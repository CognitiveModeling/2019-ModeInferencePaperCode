...
public class AdamLearner implements ErrorBasedLearner {
	...
	public AdamLearner(final int numValues, final double learningRate) {
	...
	public AdamLearner(final int numValues, final double learningRate, final double beta1, final double beta2) {
		this.numValues = numValues; 
		this.learningRate = learningRate; 
		this.beta1 = beta1;
	...
	public final double[] learningIteration(final double[] derivatives) {
		for(int i=0; i<this.numValues; i++) {
			...
			this.values[i] -= this.learningRate * (this.valueUpdates[i] / (1. - beta1pt)) 
	...
	public final double[] learningIterationBounded(final double[] derivatives, final double min, final double max) {
		...
		for(int i=0; i<this.numValues; i++) {
			...
			double change = this.learningRate * (this.valueUpdates[i] / ombeta1tau) / sqrtPlusValue;
			if(this.values[i]-change < min || this.values[i]-change > max) {
				...
				change -= diff;
				this.valueUpdates[i] = diff * ombeta1tau / this.learningRate * sqrtPlusValue;
			}
